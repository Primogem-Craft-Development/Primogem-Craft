...
public class JingyanshutongyongsuijijingyanjiangliProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
		...
		if (!itemstack.getOrCreateTag().getBoolean("jingyanshu_naijiu")) {
			if (Math.random() < 0.6) {
				itemstack.setDamageValue((int) (itemstack.getMaxDamage() - (itemstack.getMaxDamage() - 1) * Mth.nextDouble(RandomSource.create(), 0.1, 0.5)));
			} else {
				itemstack.setDamageValue((int) (itemstack.getMaxDamage() - (itemstack.getMaxDamage() - 1) * Mth.nextDouble(RandomSource.create(), 0.1, 1)));
			}
