			...
			if (itemstack.getItem() == PrimogemcraftModItems.AXTJ.get() && Math.random() <= 0.1) {
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.glass.hit")), SoundSource.PLAYERS, (float) 0.5, 1);
					} else {
